...
| `kafka.producerBatchBytes`                  |      string       |    no    |  1mb   | Maximum size (bytes) for batch. If exceeded, flush will be triggered.                                           |                                                                                                                                                                          |
| `kafka.skipOversizedMessages`               |       bool        |    no    |  false  | Skip oversized Kafka write errors instead of panicking.                                                        | When enabled, oversized messages are removed before flush and `MessageSizeTooLarge` errors are logged instead of panicking; valid messages in the same partition are still delivered. |
| `kafka.maxMessageBytes`                     |      string       |    no    |  1mb   | Maximum encoded Kafka record size before flush.                                                                 | Used with `skipOversizedMessages` to pre-filter messages that would cause the broker to reject an entire partition batch. Should match topic `message.max.bytes`.       |
